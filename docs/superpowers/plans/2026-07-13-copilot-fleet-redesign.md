...
- [ ] **Step 1: `validate.yml`**: keep the 3-OS matrix and the single `Gate A` step (validator v2 + Gate B + selected-mode projection + guard/broker/wiring suites + tripwires + eval structure); add `release` to push/PR triggers. Setup/refresh does not exist until Task 43 and is added to Gate A there. Real hostile-runtime viability remains manual evidence, never a faked CI result.
...
- [ ] **Step 3a: Brokered-client version control.** Can the exact VS Code build and Copilot extension version be managed/pinned with automatic updates blocked on this engineer's machine? Prove an attempted update cannot replace either while brokered wrappers are active. If not, `execution_mode` is safe regardless of the hook probes; a scheduled detector cannot close a window created by a client that stopped invoking it.
...
- [ ] **Step 3c: Probe the active-plugin fingerprint contract.** Install the spike/candidate plugin, trigger an update, and prove the typed Task-38 adapter identifies the **currently active** plugin root—not merely a stale cache directory—with manifest version, candidate commit SHA, runtime-tree manifest, and exact loaded-tree contents. Record behavior when absent, disabled, dirty, updated, and when a stale cache remains. If VS Code exposes no stable active-instance root/fingerprint, plugin delivery is **STOP** and fallback is used. Do not weaken this to "some cached plugin with the right version exists."
- [ ] **Step 3d: Establish the plugin-recovery prerequisite.** On the real client, verify and record the noninteractive settings/extension operations and reload signal that Task 43's updater would need to disable/unregister the active plugin and activate fallback-safe paths. This is feasibility evidence only—the installed updater does not exist until Task 43. A plugin result remains provisional until Task 47 runs the built updater through the complete atomic transition; unsupported operations make plugin delivery **STOP** immediately.
- [ ] **Step 4: Decide the candidate channel and requalify the execution mode on the team client.** Choose plugin as a **provisional candidate** only when the three product/org gates are green, Step 3c proves the active fingerprint, and Step 3d finds the required recovery operations; otherwise choose fallback. Run Task 38's complete hostile-workspace/current+nested-identity/outer-spawn/`updatedInput` suite through that selected channel on this engineer's actual client and policy. If canonical mode is brokered and any assertion fails, atomically change it to `safe`, regenerate the Copilot, Claude, safe-recovery, and runtime-tree projections, and rerun Gate A plus direct+nested denial; this task may downgrade but never upgrade the Phase-4 result. Record the candidate channel, final execution mode, raw evidence, and versions in `channel-decision.md`. Task 47 alone may finalize a plugin channel after exercising the installed updater; failure there selects fallback or canonical safe mode. **Whatever the product/org answers, a safe-mode fleet still ships.** Commit the decision and any downgrade.
- [ ] **Step 5: Pre-create the protection boundary, not the branch.** Create a repository ruleset targeting the future `release` ref before that ref exists: pull requests/review required, exact Gate-A status context required, force-push and deletion forbidden, administrators included. Do **not** run `git branch release` from the Phase-5 feature checkout; that would seed a production ref with unmerged code and violate the spec's promotion rule. The plugin adapter is tested against the non-production spike/candidate. Task 47 creates `release` exactly once from the reviewed green `origin/main` Phase-5 SHA after CODEOWNERS and full protection exist, then performs the real install test.
...
- [ ] **Step 2: Preflight the full production boundary.** `git` and `gh auth status` succeed; VS Code settings found; absolute owner-approved outer executor, hash utility, isolated-capable Python, and every brokered tool binary found outside mutable roots. Brokered mode also requires Task 42's enforceable exact VS Code/Copilot version pin and blocked auto-update state; otherwise setup regenerates/selects safe mode before discovery. Maintain a verification clone with canonical origin, fetch `release`, and require a clean `HEAD == origin/release`. Through `gh api`, require **all** Task-44 controls: review count, Code Owner review, exact Gate-A context, administrators enforced, force-push/deletion disabled. The pre-CODEOWNERS Task-42 ruleset is sufficient only for hermetic development fixtures; production setup/refresh refuses it.
...
- [ ] **Step 5: `-Refresh` is the only updater.** The installed scheduled launcher first verifies the installed setup hash, then acquires an exclusive lock. It fetches canonical protected `origin/release`, verifies full protection, stages the exact commit/tree/runtime/updater, and requires candidate SHA equality. **On discovering remote != recorded SHA, expired 26-hour freshness, plugin lag/ahead, client-version skew, or any partial transition, atomically disable fleet discovery/switch to the verified safe projection before returning nonzero**; a maintenance record alone cannot protect a client that stopped invoking its hook. Clear maintenance and restore brokered discovery only after active source, copied guard/broker/updater, runtime-tree contents, protected release, pinned client versions, and full boundary probes converge. Planned VS Code/Copilot upgrades first switch to safe mode while the old client still runs, then unpin/update/requalify; brokered wrappers are never active during the upgrade. For fallback, atomically switch JSONC paths to the completed versioned tree; for plugin, wait/retry until the active plugin fingerprint equals release. Self-update the installed setup and scheduled embedded hash in the same locked transaction. Never schedule bare `git pull`.
- [ ] **Step 6: `-Verify` is a gate.** Report channel; proof that the opposite channel and colliding workspace definitions are inactive; execution mode; out-of-band bootstrap SHA/hash; active root/version/SHA/full-tree digest; fetched protected release + age; every protection field; installed updater/guard/broker/interpreter/tool hashes; isolated Python state; hook invocation scope/precedence and liveness in brokered mode; audit health; distributed-hook emptiness; skills load; `gh auth`; and client probe skew. Exercise healthy non-fleet isolation, guarded broker allow/deny, forced post-launch integrity deny, current/nested/source-qualified identity, and maintenance state—or, in safe mode, direct/nested execute denial. Any mismatch exits nonzero and the fleet is not enabled.
- [ ] **Step 7: Deterministic setup/refresh tests.** `scripts/test_setup.py` covers wrong origin, wrong/dirty HEAD, incomplete/full branch protection, bootstrap hash mismatch, a poisoned PATH/profile/alias around the pre-execution verifier, symlinked/tampered installed updater, schedule-command materialization, JSONC comments/trailing commas, lock contention, every transaction fault boundary, tree tampering/unexpected/colliding files, plugin lag/ahead/stale cache, dual registration, plugin→fallback-safe and fallback-safe→plugin exclusive switching, transition failure before/after each atomic boundary, remote skew selecting safe recovery, freshness expiry/network failure, fallback version switch, and recovery only after requalification. Tests use fixtures/mocks; there is no production-bypass flag. Add this suite to `scripts/gate_a.py` as a named structural step in the same task; no earlier task may claim it is already present. Fixture success cannot replace Task 42 Step 3d's real-client feasibility evidence or Task 47's installed-updater transition proof.
...
### Task 44: Full protection, CODEOWNERS, versioning
...
```
  ...
  Optional exceptions may follow only for paths explicitly proven non-executable/non-prompt/non-gate; the initial file has none.
- [ ] **Step 2: Configure and fully inspect protection** on `main` and the future `release` pattern: at least one PR review, **Require review from Code Owners**, exact required Gate-A status context, force-push and deletion forbidden, and administrators included/no bypass. Query the complete branch-protection/ruleset response and assert every field—`enforce_admins.enabled: true` alone is insufficient. Because CODEOWNERS is not on the base branch until the Phase-5 PR lands, that bootstrap PR requires an explicit recorded approval from the named maintainer even if GitHub does not yet request it automatically.
- [ ] **Step 3: Post-merge activation check (performed in Task 47).** After the Phase-5 PR lands on `main`, open a probe PR touching a root test file and show that the named Code Owner is requested and merge is blocked without that review. Then create `release` from that exact reviewed `origin/main` SHA and repeat the full protection query. No setup/refresh trusts release before both checks pass.
- [ ] **Step 4: Versioning discipline**, recorded in CONTRIBUTING (Task 45): bump canonical `plugin.version`, regenerate both manifests + Copilot safe-recovery + runtime-tree projections, and require `--check` on every `main`→`release` promotion; never edit projections directly. Skill rename/skew rules remain unchanged.
- [ ] **Step 5:** Commit; the Phase-5 close PR carries the one-time maintainer bootstrap approval.

### Task 45: README, CONTRIBUTING, the rollback runbook, RESEARCH.md

- [ ] **Step 1: `README.md` rewritten**: what this is (5 agents / 26 skills, VS Code Copilot); install begins by comparing the maintainer-announced protected-release SHA + setup hash before executing setup, then marketplace trust or fallback selection. Explain the selected `execution_mode`: brokered includes a reviewed machine-local hook/broker only because Task 38 proved an unshadowable scope; safe mode omits execute and asks humans to run commands. Portable hooks stay empty. Include `setup.ps1 -Channel fallback`, maintainer name, generated fleet table, and repo-mechanics pointer.
...
- [ ] **Step 3: `docs/runbooks/fleet-rollback.md`**: revert the offending `release` commit, then bump canonical `plugin.version`, regenerate both manifests plus the safe-recovery and runtime-tree projections, and require `--check` (a plain revert can restore an equal-or-lower version and silently fail to update clients). The installed updater detects the protected-release change, switches discovery to the verified safe projection while plugin/fallback state is stale, performs `-Refresh`, and must pass `-Verify` before re-enabling brokered mode; a marketplace refresh may be requested to shorten availability lag but never substitutes for those checks. Announce, verify, and link the postmortem.
...
- [ ] **Step 1: Prepare the pilot release candidate.** Bump canonical `plugin.version` to `0.9.0`, regenerate both runtime projections + Copilot safe-recovery + runtime-tree manifest, and run Gate A + Gate C (security reviewer on setup/bootstrap/refresh/broker; conformance on Sections 1, 5, 9). Rebase and open the Phase-5 PR to `main`; require the named maintainer's explicit bootstrap approval because CODEOWNERS is being introduced by this PR. Merge only after the exact Gate-A check is green.
- [ ] **Step 2: Bootstrap `release` from reviewed main, never from a feature branch.** Fetch canonical origin; require clean local `HEAD == origin/main`; record that SHA and its successful required check. Confirm the future-release ruleset is active, then create `refs/heads/release` **at that exact SHA** as the one-time bootstrap. Query every Task-44 protection field and perform the Code-Owner probe PR; if any control is absent, delete no refs and do not onboard. This is the only creation exception—subsequent changes are reviewed `main → release` promotions.
- [ ] **Step 3: Publish and test the real bootstrap; finalize the channel.** Publish the protected release SHA + setup hashes through the maintainer-approved channel. On the pilot machine, verify them out of band and run the candidate channel end-to-end. For a plugin candidate, use the now-installed updater to force plugin→fallback-safe recovery and prove it noninteractively disables/unregisters the active plugin, atomically activates/reloads the preverified fallback-safe paths, and leaves neither duplicate nor brokered definitions active before returning; then prove fallback-safe→plugin restoration occurs only after complete requalification. Capture active-source fingerprints and tool inventories before, during, and after both transitions. Any failure makes plugin delivery **STOP** and this step reruns through fallback if its boundary passed, otherwise canonical safe mode. Only then record the final channel and require `-Verify` clean, including execution mode, active-tree/source/freshness handshake, full protection, installed updater schedule, and hostile-workspace boundary result. Paste the evidence into the PR/decision record. Phase 6 cannot start on the pre-Phase-5 baseline or a provisional plugin result.
...
- [ ] **Step 1:** Bump canonical `plugin.version` to `1.0.0`, regenerate both manifests plus safe-recovery/runtime-tree projections, require `--check`, then PR `main` → `release`. Announce in the team channel; onboarding per README.
- [ ] **Step 2: Week-one watch — with an instrument, not just a trigger [graft from PR #61]:** mid-week, run `setup.ps1 -Verify` on **at least one teammate's machine** (not the owner's), inspect broker audit/maintenance/freshness records or safe-mode omission evidence, and take one spot routing report. Any Section-8 regression → rollback. Without a named check, silence reads as health.
