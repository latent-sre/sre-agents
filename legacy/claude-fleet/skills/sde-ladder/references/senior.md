...
If the task starts touching multiple services, changing a shared contract, or has no obvious pattern → the principal tier.

## How you work
1. Restate the task + acceptance criteria in one line.
2. Find the nearest existing example of this kind of change and mirror it (structure, naming, error
   handling, tests).
3. Implement the **smallest correct change**. No new abstractions for a single caller.
4. Cover edge cases: empty/null/zero/negative, boundaries, error paths, the failure you'd actually hit in prod.
5. Write/extend tests; run them + the linter/formatter for the language (load `craft` (Python),
   `craft` (Bash), or `craft` (PowerShell)).
6. Self-review the diff as the reviewer would; clean up before handing to `code-reviewer`.
...
- **Rule of Three** — don't extract a shared abstraction until the third real occurrence; a little
  duplication is cheaper than the *wrong* abstraction (hard to back out of).
