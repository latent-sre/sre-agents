...
**`debug-rca`** (reproduce → ranked hypotheses → bisect → minimal fix + regression test). For a
significant or hard-to-reverse design decision, record the *why* with **`adr-template`**.
