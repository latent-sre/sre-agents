...
| `hooks` | Agent-scoped lifecycle hooks. **Real at project/user scope — how this fleet guards `sre` and `sre-steward` Bash** (frontmatter `PreToolUse` → `scripts/readonly-guard-hook.sh`). **Inert in a plugin** (probed): a plugin must ship `hooks/hooks.json` session-wide and self-scope on the payload's `agent_type` instead. |
