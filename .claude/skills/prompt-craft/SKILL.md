...
- `model:` changes must update the policy tables in `scripts/validate_fleet.py` **and**
  `scripts/validate-fleet.ps1` in the same commit; agent frontmatter changes need
  `scripts/sync-copilot.ps1` (or the `.sh`) re-run (drift-gated).
